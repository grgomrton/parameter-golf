...
`mlx` is only available on macOS with Apple Silicon. It will not install on Windows or on non-Apple machines.

The checked-in CUDA training path, `train_gpt.py`, currently requires an NVIDIA GPU and a CUDA-enabled PyTorch install. On Windows, use a 64-bit Python version supported by PyTorch wheel builds (for example Python 3.12 or 3.13 rather than Python 3.14), then install the CUDA-enabled PyTorch packages in a fresh environment. If you do not have Apple Silicon or an NVIDIA GPU, we recommend jumping straight to cloud GPUs with Runpod.
...
This populates `.\data\datasets\fineweb10B_sp1024\` and `.\data\tokenizers\`.
By default this downloads the full validation split plus 80 training shards (8B tokens). For a smaller local smoke subset, pass `--train-shards 1`, for example `python data/cached_challenge_fineweb.py --variant sp1024 --train-shards 1`.
